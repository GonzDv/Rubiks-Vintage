		...
		return (
			<div className='min-h-screen bg-[#F5F1EB] flex flex-col items-center justify-center gap-4 text-black/30'>
				...
				<p className='text-xs text-black/20'>
					Si realizaste el pago, contáctanos para confirmar
					tu pedido.
				</p>
				<button
					onClick={() => router.push('/')}
				...
				<div className='bg-white rounded-2xl p-6 border border-black/5 shadow-sm text-left space-y-3'>
					<div className='flex items-center gap-2 mb-4'>
						<PackageCheck
							...
							className='text-[#A07F3A]'
						/>
						<p className='text-[9px] uppercase tracking-[0.3em] font-bold text-black'>
							Detalles del Pedido
						</p>
					</div>
					...
					{orderData?.orderId && (
						<div className='flex justify-between text-xs'>
							...
							<span className='font-mono text-black/50 text-[10px]'>
								{orderData.orderId
									.slice(0, 8)
									.toUpperCase()}
							</span>
					...
					{orderData?.total && (
						<div className='flex justify-between text-xs border-t border-black/5 pt-3 mt-3'>
							<span className='text-black/30 uppercase tracking-widest text-[9px]'>
								Total pagado
							</span>
